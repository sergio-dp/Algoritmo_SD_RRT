...
class algoritmo:
    ...
    def conectar_a_grafo(self, p):
        ...
        if punto_conexion:
            ...
            self.roadmap.dibujar_arista(punto_conexion, segmento_conexion[0], self.mapa)
            self.roadmap.dibujar_arista(punto_conexion, segmento_conexion[1], self.mapa)
            ...
            if not self.iteracion % 100: # Cada 100 iteraciones intentamos encontrar un camino
                ...
                if origen_conectado and destino_conectado:
                    while True:
                        ...
                        if camino:
                            if self.roadmap.filtrar_camino(camino, self.mapa):
                                camino.insert(0, self.origen)
                                camino.append(self.destino)
                                self.mapa.dibujar()
                                self.roadmap.dibujar(self.mapa)
...
class Roadmap:
    ...
    def dibujar(self, mapa):
        for v, edges in self.vertices.items(): # Dibuja las aristas del arbol
            self.dibujar_nodos(v)
            for p in edges:
                self.dibujar_arista(v,p,mapa)
        auxiliar.pygame.display.update()
    
    # FUNCION PARA DIBUJAR LAS ARISTAS DEL ROADMAP
    def dibujar_arista(self, p1, p2, mapa):
        if mapa.comprobar_segmento(p1, p2):
            auxiliar.pygame.draw.line(self.canvas, auxiliar.color_arista, p1[:2], p2[:2], auxiliar.grosor_arista)
        else:
            auxiliar.pygame.draw.line(self.canvas, auxiliar.amarillo, p1[:2], p2[:2], auxiliar.grosor_camino)
    ...
    def añadir_vertice(self, qn, mapa):
        ...
        for vecino in vecinos:
            ...
            self.dibujar_arista(qn, vecino[0], mapa)
        return True
    ...
    # FUNCIÓN QUE SIRVE PARA ELIMINAR DEL CAMINO ENCONTRADO ARISTAS QUE CHOCAN CON OBSTÁCULOS
    def filtrar_camino(self, camino, mapa):
        pi = camino[0]
        for pf in camino[1:]:
            if not mapa.comprobar_segmento(pi,pf):
                self.eliminar_arista(pi,pf)
                return False
            pi = pf
        return True
    
    # FUNCIÓN QUE SIRVE PARA ELIMINAR UNA ARISTA DEL ROADMAP
    def eliminar_arista(self, v1, v2):
        if v1 in self.vertices and v2 in self.vertices: # Aseguramos que ambos vértices existen
